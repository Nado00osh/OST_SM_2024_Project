...
# Load the pre-trained pipeline containing both StandardScaler and One-Class SVM model
with open('../Abnormal/one_class_pipeline_ocsvm.pkl', 'rb') as file:
        ...
        while True:
            ...
            if new_data:
                ...
                for data in new_data:
                    ...
                    if 'Date' in data:
                        del data['Date']  # This removes the 'Date' column from the data
                    
                    # Extract feature values
                    feature_values = [data[feature] for feature in data.keys()]
                    feature_array = np.array([feature_values])

                    # Predict using the loaded pipeline (StandardScaler + One-Class SVM model)
                    prediction = pipeline.predict(feature_array)[0]

                    # Add prediction to the original data
                    data['prediction'] = prediction  # Store 1 for normal, -1 for abnormal (One-Class SVM)

                    # Prepare the data for storage in InfluxDB
                    point = Point("abnormal_data_one_class").tag("bucket", bucket)
